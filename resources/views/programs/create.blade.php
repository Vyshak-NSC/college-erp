            ...
            <div class="bg-white dark:bg-gray-800 shadow sm:rounded-lg p-6">
                <form method="POST" action="{{ route('programs.store') }}">
                    ...
                    <div class="mb-4">
                        ...
                        <select name="total_semesters" id="total_semesters" class="block w-full mt-1  shadow-sm rounded-md
                                                                   border-gray-300 dark:border-gray-700 
                                                                   dark:bg-gray-900 dark:text-gray-300 
                                                                   focus:border-indigo-500 dark:focus:border-indigo-600 
                                                                   focus:ring-indigo-500 dark:focus:ring-indigo-600">
                            ...
                            @for ($i=1; $i<9; $i++)
                                <option value="{{ $i }}" @selected(old('total_semesters' === $i))>{{ $i }}</option>
                            @endfor
                        </select>
                        <x-input-error :messages="$errors->get('total_semesters')" class="mt-2" />
                    </div>
